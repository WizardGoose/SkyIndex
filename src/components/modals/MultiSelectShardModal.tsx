...
export const MultiSelectShardModal: React.FC<MultiSelectShardModalProps> = ({ isOpen, onClose, shards, onDone, initialSelections = new Map(), ownedAttributes = new Map() }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/60" onClick={onClose}>
      <div className="bg-slate-800 rounded-md shadow-2xl w-full max-w-5xl max-h-[90vh] sm:max-h-[85vh] flex flex-col border border-slate-700" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-2 bg-slate-700/50 border border-slate-600/50 rounded-md text-white placeholder-slate-400 ${FOCUS}`}
                autoFocus
              />
            </div>
            ...
            {filteredShards.map((shard) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-2 p-2.5 rounded-md transition-all duration-300 border cursor-pointer ${
                ...
                >
                  ...
                  {isSelected && (
                    <input
                      ...
                      className={`w-14 px-2 py-1 text-sm bg-slate-700 border border-slate-600 rounded text-white text-center flex-shrink-0 ${FOCUS}`}
                    />
                  )}
                </div>
              );
