...
export const InventoryManagementModal: React.FC<InventoryManagementModalProps> = ({
  ...
  /**
   * What the last import could NOT read, so the banner can say so.
   *
   * Hypixel omits `attributes.stacks` for a member who has never fused
   * anything, and the two cases - "fused nothing" and "we could not read your
  ...
  /**
   * The profile Hypixel marks as current, first; everything else in the order
   * Hypixel listed it.
   *
   * This used to sort by `last_save`, most recent first. Hypixel's v2 profiles
   * payload has no such field - see `shards/profileImport.ts` - so that sort
   * was ordering by a column of zeros. `selected` is real data and answers the
   * same question ("which one am I on"), so it is what the list leads with.
   * `Array.prototype.sort` is stable, so the rest keeps Hypixel's order.
   */
  const sortedProfiles = useMemo(() => {
    if (!profileData) return [];
    return [...profileData.profiles].sort((a, b) => Number(b.profile.selected) - Number(a.profile.selected));
  }, [profileData]);

  /*
   * No save date on the options. `last_save` is pinned to 0 by the importer,
   * because the v2 profiles payload does not carry one, so every option read
   * "Unknown" in the same place. A label that says nothing on every row is not
   * a label; name and game mode are the two things that tell the profiles
   * apart.
   */
  ...
  const importProfileData = (profileData: ProfileData, username: string) => {
     ...
     * and is skipped entirely when Hypixel did not send them.
     *
     * Writing zeros here on an unreadable profile would silently wipe the
     * player's hand-entered attribute levels and reset the nine shard-level
     * fields on the calculator form. An import that quietly deletes data is
     * worse than an import that says it could not read part of it.
     */
    if (profileData.attributesRead) {
      const shardLevels: Record<string, number> = {};
      for (const { shardId, rarity, formKey } of shardLevelMapping) {
        const fusedCount =
          profileData.attributes.find((attr) => attr.id.toUpperCase() === shardId.toUpperCase())?.level ?? 0;
  ...
  const handleImport = async () => {
    ...
    try {
      // The catalogue is what turns Hypixel's `SHARD_GROVE` into our `C1`.
      // Importing before it has loaded would map nothing at all, so say so
      // rather than hand back an empty inventory that looks like an answer.
      if (shardsArray.length === 0) {
        setError("Shard data is still loading. Give it a second and try again.");
        return;
      }
      ...
      // Hypixel's own `selected` flag, not a timestamp: the payload carries no
      // `last_save` to pick "most recent" from.
      const chosen = data.profiles.find((p) => p.profile.selected) ?? data.profiles[0];
      setSelectedProfileId(chosen.profile.profile_id);
      importProfileData(chosen, data.username);
    } catch (err) {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div
        className="bg-slate-900 border border-slate-700 rounded-md shadow-2xl max-w-3xl w-full max-h-[85vh] overflow-hidden flex flex-col"
        ...
        <div className="p-4 border-b border-slate-700 bg-slate-800/50 flex-shrink-0">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-1">
              ...
              <button onClick={onClose} className="p-2 hover:bg-slate-700 rounded-md transition-colors cursor-pointer">
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className={`w-full pl-9 pr-3 py-2 bg-slate-800 border border-slate-600 rounded-md text-white text-sm ${FOCUS}`}
                />
              </div>
              <button
            ...
            {/* Error message */}
            {error && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-md p-2 flex items-start gap-2">
            ...
            {/*
              What the import could not read.

              Sits under the success banner rather than replacing it, because
              the shard counts really did come through; it is only the fused
                ...
                <span className="text-amber-200 text-sm">
                  ...
                  {importGaps.unmappedShards > 0 && (
                    <>
                      ...
                      {importGaps.unmappedShards === 1 ? " is" : " are"} not in this site&apos;s shard data yet and{" "}
                      {importGaps.unmappedShards === 1 ? "was" : "were"} skipped.
                    </>
                  )}
                </span>
              </div>
                ...
                {sortedProfiles.length > 1 && (
                  <div className="relative">
                    <select
                      ...
                      className={`pl-2 pr-6 py-1 bg-slate-700 border border-slate-600 rounded text-white text-xs appearance-none cursor-pointer ${FOCUS}`}
                    >
                      ...
                      <div key={shardId} className={`bg-slate-800/50 border ${shardRarityBorder} rounded-md p-2 space-y-2 ${disabled ? "opacity-50" : ""}`}>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="0"
                            className={`w-full px-2 py-1 bg-slate-700 border border-slate-600 rounded text-white text-sm ${FOCUS}`}
                          />
                        </div>
                      </div>
                    );
