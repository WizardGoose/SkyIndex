          ...
          <div className={`flex ${detailed ? 'flex-col' : 'flex-row'} gap-1.5 flex-shrink-0 self-center`} onClick={(e) => e.stopPropagation()}>
            {!detailed && (
              <button
                ...
                className={`flex items-center justify-center px-1.5 py-1.5 cursor-pointer text-sm text-center bg-red-500/20 border border-red-500/30 rounded-md text-red-400 hover:bg-red-500/30 hover:text-red-300 transition-colors duration-150 ${FOCUS}`}
            ...
            <input
              ...
              className={`w-20 px-2 py-1 text-sm text-center bg-white/5 border border-white/10 rounded-md text-white placeholder-slate-400 transition-colors duration-200 ${FOCUS}`}
            />
            {detailed && (
              <button
                type="button"
                title="Reset to default"
                className={`flex items-center justify-center gap-1 px-2 py-1 cursor-pointer text-xs text-center bg-red-500/20 border border-red-500/30 rounded-md text-red-400 hover:bg-red-500/30 hover:text-red-300 transition-colors duration-150 whitespace-nowrap ${FOCUS}`}
                onClick={() => onRateChange(shard.key, defaultRate)}
                tabIndex={0}
              >
